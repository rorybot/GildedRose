...
class GildedRose
  ...
  def update_quality()
    ...
    @items.each do |item| #iterates over every item
      ...
      if item.name != "Aged Brie" and item.name != "Backstage passes to a TAFKAL80ETC concert" #excludes Aged Brie from quality reductiondue to special date increase of quality; excludes pass from quality reduction due to special date increase of quality
